const portfolio = {
  architecture: {
    ...
    projects: [
      ...
      {
        slug: 'millennium-seed-bank', no: 'A—04', title: 'Millennium Seed Bank', date: '2025', type: 'Research Study',
        role: 'Journey Continues · Research Study',
        question: 'How can architecture make preservation and time legible?',
        ...
        evidence: ['Preservation timelines mapped across collection, storage, and public learning', 'Parti models and program diagrams made an invisible ecological process spatial', 'The study became a bridge into the next set of architectural questions'],
      ...
      {
        slug: 'call-of-the-sea', no: 'A—07', title: 'Call of the Sea', date: '2025', type: 'Competition',
...
if (lensKey === 'architecture') {
  lens.projects = lens.projects.slice().sort((a, b) => recencyScore(b.date) - recencyScore(a.date)).map((item, index) => ({ ...item, no: `A—${String(index + 1).padStart(2, '0')}` }));
}
...
const mediaByProject = {
  'convergence-environmental-middle-school': ['p2-final-final-perspective-1-for-print.webp', 'p2-field-model-large.webp', 'p2-field-model-rule-set-pattern.webp', 'p2-rule-set.webp', 'p2-research-map.webp', 'p2-map.webp', 'p2-site-model.webp', 'p2-physical-model2.webp', 'p2-plan-1-super.webp', 'p2-plan-for-print-2.webp', 'p2-sections.webp', 'p2-资源-6section.webp', 'p2-site-plan-and-site-section.webp'],
  'the-tinkerers-imaginarium': ['p2-exploded-axon-16th-2.0.webp', 'p2-axon-with-call-outs.webp', 'p2-new-collage-timeline.webp', 'p2-map.webp', 'p2-field-model-large.webp', 'p2-plan-view-with-diagram.webp', 'p2-plan.webp', 'p2-sections.webp'],
  'radical-empathy': ['p2-doll-house-sectional-perspective.webp', 'p2-doll-house-exploded-axon.webp', 'p2-plan.webp', 'p2-enclosed-and-void.webp', 'p2-1.webp', 'p2-2.webp', 'p2-3.webp', 'p2-4.webp', 'p2-5.webp', 'p2-6.webp', 'p2-资源-2section-2.webp'],
  'millennium-seed-bank': ['p2-new-collage-timeline.webp', 'p2-research-map.webp', 'p2-massing-diagram2.webp', 'p2-axon-with-call-outs.webp', 'p2-plan-view-with-diagram.webp'],
  're-serv-oir': ['reservoir-hero.jpg', 'reservoir-axon.jpg', 'reservoir-ground-plan.jpg', 'reservoir-mezzanine.jpg', 'reservoir-first-floor.jpg', 'reservoir-long-section.jpg', 'reservoir-climate-section.jpg', 'reservoir-site-plan.jpg', 'reservoir-circular-map.png'],
  'how-to-build-a-ruin': ['ruin-hero.jpg', 'ruin-timeline.jpg', 'ruin-process.jpg', 'ruin-growth.jpg', 'ruin-circulation.jpg', 'ruin-parti-01.jpg', 'ruin-parti-02.png', 'ruin-parti-03.jpg', 'ruin-parti-04.jpg'],
...
const mediaCaptionsByProject = {
  ...
  'millennium-seed-bank': ['Journey timeline — preservation, collection, and public learning', 'Research map — ecological systems and the seed bank’s wider context', 'Massing diagram — an invisible preservation process made spatial', 'Program axonometric — storage, research, and public encounter', 'Plan diagram — the study’s next architectural question'],
  're-serv-oir': ['Community ecology center — an obsolete water tank reclaimed as civic ground', 'Transformation sequence — drain, cut, reposition, frame, reclaim', 'Ground floor — courtyard, rain gardens, and public learning spaces', 'Mezzanine — workshops, library, and shared study spaces', 'First floor — programs gathered around the central courtyard', 'Long section — the retained reservoir walls and new timber frame', 'Environmental system — water, air, and thermal performance', 'Site plan — the center connected to Portland’s civic systems', 'Circular map — infrastructure, neighborhood, and movement'],
  'how-to-build-a-ruin': ['Seed bank — a research institute growing with what it protects', 'Timeline — preservation, climate, and the long future of the site', 'Rice as system — a spine, branch, harvest, and cycle', 'Rice as material — agricultural waste translated into UHPC research', 'Circulation — the seed bank as a mediating landscape node', 'Parti study — energy, ecology, and agricultural resilience', 'Program study — rooms organized along a productive spine', 'Material study — the building as a future ruin'],
...
document.getElementById('project-root').innerHTML = `
  <section class="project-gateway" id="top">
    ...
    <div class="project-gateway-heading">
      <span>${project.no} / ${project.type}</span>
      <h1>${project.title}</h1>
    ...
    <dl class="project-gateway-meta">
      <div><dt>Role</dt><dd>${project.role}</dd></div>
      <div><dt>Time</dt><dd>${project.date}</dd></div>
      <div><dt>${projectMetaLabel}</dt><dd>${projectMetaValue}</dd></div>
    </dl>
    ${lensKey !== 'pm' ? `<figure class="project-hero-media"><img src="../assets/portfolio/${projectMedia[0]}" alt="${mediaAlt(0)}" fetchpriority="high"></figure>` : ''}
    <div class="project-aperture aperture-${lensKey}" aria-hidden="true">
      <span></span><span></span><span></span><span></span><span></span>
      <strong>${project.no}</strong><em>${lens.code}</em>
    </div>
    <a class="project-enter" href="#case-study"><span>Enter project</span><i>↓</i></a>
  </section>

  <section class="case-framing" id="case-study">
  ...
  <section class="case-sections" aria-label="Project case study">
  ...
  <section class="project-gallery" aria-label="Additional project visuals">
    <header><span>05 / VISUAL RECORD</span><h2>Additional drawings, studies, and artifacts.</h2></header>
    <div class="project-gallery-grid">${projectMedia.slice(1 + sectionCount).map((image, index) => `
